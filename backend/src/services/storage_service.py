...
class StorageService:
    def __init__(self):
        ...
        self.upload_dir = os.path.join(self.base_dir, self.upload_subdir) if self.base_dir else self.upload_subdir
    ...
    async def save_model_file(self, file: UploadFile, platform: str) -> str:
        model_subdir = "models"
        model_dir = os.path.join(self.base_dir, model_subdir) if self.base_dir else model_subdir
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        
        platform_dir = os.path.join(model_dir, platform)
        if not os.path.exists(platform_dir):
            os.makedirs(platform_dir)
        
        file_extension = os.path.splitext(file.filename)[1].lower()
        unique_filename = f"{uuid.uuid4()}{file_extension}"
        full_path = os.path.join(platform_dir, unique_filename)
        relative_path = os.path.join(model_subdir, platform, unique_filename)
